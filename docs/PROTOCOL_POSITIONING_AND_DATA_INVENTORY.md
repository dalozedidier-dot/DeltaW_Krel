...
No external experimental dataset has been imported into this repository yet.
The entries below are a prioritized inventory for future reproducible ingestion.
...
| External experimental data | future ingestion | not imported | not claimed |
...
| 5 | Cao et al., "Semi-device-independent certification of indefinite causal order in a photonic quantum switch", Optica 10, 561 (2023) | DOI `10.1364/OPTICA.483876`, arXiv `2202.05346` | experimental semi-DI switch; strong target for external-data comparison | paper located; repository/data link still to be resolved |
...
Until raw experimental data are ingested and parsed reproducibly, the site and
manuscript should say:

```text
External-data reanalysis is planned and inventoried, but not yet claimed.
```
